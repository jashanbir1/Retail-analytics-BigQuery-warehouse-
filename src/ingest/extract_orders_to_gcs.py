...
def build_gcs_blob_name() -> str:
    ...
    return f"raw/shopify/orders/extract_date={extract_date}/orders.json"


def fetch_orders_from_shopify(access_token: str) -> dict:
    headers = {
...
def main() -> None:
    ...
    orders_payload = fetch_orders_from_shopify(access_token)
    ...
    order_count = len(orders_payload.get("orders", []))
    print(f"Orders extracted: {order_count}")
