...
def build_gcs_blob_name() -> str:
    ...
    return f"raw/shopify/customers/extract_date={extract_date}/customers.json"


def fetch_customers_from_shopify(access_token: str) -> dict:
    headers = {
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"Shopify customers request failed.\n"
...
def main():
    ...
    customers_payload = fetch_customers_from_shopify(access_token)
    blob_name = build_gcs_blob_name()

    upload_json_to_gcs(
        bucket_name = GCS_BUCKET_NAME,
        blob_name = blob_name,
        payload = customers_payload,
    )

    customer_count = len(customers_payload.get("customers",[]))
